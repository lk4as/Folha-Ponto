...
with col4:
    cargo_func = st.text_input("Função / Job Title", "Estagiário - Dep. DP Assurance")

col5, col6 = st.columns(2)
with col5:
    base_opt = st.selectbox("Base", ["Rio", "Açu", "Macaé", "Guaxindiba"], index=1)
...
if tipo_horario.startswith("Fixo"):
    ...
    st.write("Defina a entrada de cada dia e o horário de almoço padrão.")
    cols = st.columns(5)
...
feriados_str = st.text_input("Dias de feriado (separe por vírgula, ex: 15, 25)", "")
feriados = []
if feriados_str:
    try:
        feriados = [int(x.strip()) for x in feriados_str.split(",") if x.strip().isdigit()]
    except:
        st.error("Erro ao ler feriados. Use apenas números separados por vírgula.")
        ...
        for table in doc.tables:
            for row in table.rows:
                try:
                    ...
                    # Prioridade: Feriado > Fim de Semana > Dia Útil
                    if dia in feriados:
                        for i in cells_alvo: 
